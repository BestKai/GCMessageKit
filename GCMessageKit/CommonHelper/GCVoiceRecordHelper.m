...
@interface GCVoiceRecordHelper () <AVAudioRecorderDelegate> {
    NSTimer *_timer;
...
@implementation GCVoiceRecordHelper
...
- (void)getVoiceDuration:(NSString*)recordPath {
    ...
    if (error) {
        ...
        self.recordDuration = [[NSString alloc] initWithFormat:@"%.1f", play.duration];
    }
}
