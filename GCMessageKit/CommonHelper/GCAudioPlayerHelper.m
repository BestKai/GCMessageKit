...
#import "GCAudioPlayerHelper.h"


@implementation GCAudioPlayerHelper
...
- (void)playAudioWithFileName:(NSString*)fileName {
    if (fileName.length > 0) {
        ...
        if (_playingFileName && [fileName isEqualToString:_playingFileName]) {//上次播放的录音
            ...
            if (fileName&&[fileName containsString:@"/var/"]) {
            ...
            }else
            {
                ...
                NSString *imagepath = [path stringByAppendingString:fileName];
                
                AVAudioPlayer *pl = [[AVAudioPlayer alloc] initWithContentsOfURL:[NSURL fileURLWithPath:imagepath] error:nil];
                pl.delegate = self;
                [pl play];
                self.player = pl;
                [[UIDevice currentDevice] setProximityMonitoringEnabled:YES];
...
- (void)setDelegate:(id<GCAudioPlayerHelperDelegate>)delegate {
...
+ (id)shareInstance {
    static GCAudioPlayerHelper *instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[GCAudioPlayerHelper alloc] init];
    });
    return instance;
}
